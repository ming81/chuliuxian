...
///conver GPu Skin
			...
			fixed4 frag (vertexout i) : SV_Target
			{
				...
				float4 xlv_TEXCOORD1 = i.posWorld;
...
////Normal
               float4 NormalTex =  tex2Dbias (sNormalSampler, float4 (uv0.xy,0, cNormalMapBias));
              ...
              // BaseNormal.xyz = ((BaseNormal.xyz * 2.0) - 1.0);
               ...
               SpecularMask1_17 = (DetailNormalMap.x * _SkinSpec_color1);
               SpecularMask2_16 = (DetailNormalMap.y * _SkinSpec_color2);
  ...
  float3 nSquared_86 = tmpvar_87;
	...
	half3 tmpvar_88;
	tmpvar_88 = half3(min (normalDirection, float3(0.0, 0.0, 0.0)));
	float4 tmpvar_89;
	tmpvar_89 = (((nSquared_86.x * cPointCloud[tmpvar_88.x]) + (nSquared_86.y * cPointCloud[(tmpvar_88.y + 2)])) + (nSquared_86.z * cPointCloud[(tmpvar_88.z + 4)]));
	
  float4 linearColor_85 = tmpvar_89;

	tmpvar_84 = (linearColor_85.xyz * (ShadowColor.x * (10.0 + ((cPointCloud[3].w * ShadowColor.z) * 100.0))));
  ...
  float4 tmpvar_95 = tex2D (sLutMapSampler, tmpvar_94);
   ...
   float tmpvar_96;
  tmpvar_96 = (shadow_10 + ((clamp (dot (normalDirection2, LightDir), 0.0, 1.0)- SunCos) * shadow_10));
  ...
  ) + tmpvar_92) + (((((tmpvar_97 * tmpvar_97) * (tmpvar_95.xyz * tmpvar_95.xyz)) * (1.0 - Ma))+ ((SunCos * shadow_10) * Ma)
  ) * Shadow));
  ...
  ) * (1.0 - Ma)) + (Roughness_18 * Ma));
   float tmpvar_100;
  tmpvar_100 = (((Roughness_18 * RoughnessOffset2) * (1.0 - Ma)) + (Roughness_18 * Ma));
   float d_101;
   float m2_102;
   float m_103;
   float3 tmpvar_104;
  ...
  tmpvar_118 = clamp (dot (normalDirection2, tmpvar_117), 0.0, 1.0);
   float tmpvar_119;
  tmpvar_119 = clamp (dot (viewd, tmpvar_117), 0.0, 1.0);
   float tmpvar_120;
  ...
  SPL = (SPL * ((cEnvStrength * EnvInfo.w) * 10.0));
		...
		* 
		...
		(((SPL * tmpvar_124) * MaksTex.z) * ((SpecularMask2_16 * (1.0 - Ma)) + Ma))
		* 
  ...
  if ((Lights0[3].w > 0.0)) {
    ...
    DiffLit_141 = (DiffLit_141 + (Lights0[1].xyz * (tmpvar_149 * Atten_145)));
     float tmpvar_151;
    tmpvar_151 = ((Roughness_var * Roughness_var) + 0.0002);
    m2_144 = tmpvar_151;
  ...
  if (((Lights1[3].w > 0.0) && (Lights1[2].w <= 0.0))) {
   ...
   float3 tmpvar_164 = (Light0.xyz - xlv_TEXCOORD1.xyz);
    ...
    L_160 = (L_160 / tmpvar_165);
     float tmpvar_166;
    tmpvar_166 = clamp (dot (MaksNormalDir, L_160), 0.0, 1.0);
    NoL_159 = tmpvar_166;
   ...
   float3 y_168 = -(L_160);
    ...
    float tmpvar_172 = clamp (dot (MaksNormalDir, normalize((viewd + L_160))), 0.0, 1.0);
...
////Emssion
  ...
  float tmpvar_177 = (CameraPosPS.w - cEmissionScale.w);

 float tmpvar_176 = clamp ((sin(((6.283185 * tmpvar_177) * cEmissionScale.z)) - 0.8), 0.0, 1.0);



  float tmpvar_178 = clamp ((sin(((6.283185 * tmpvar_177) * cEmissionScale.z)) - 0.8), 0.0, 1.0);

  float3 finalEmssion = (EmssionMap.xyz * (EmssionMap.w * ((cEmissionScale.x * (1.0 - tmpvar_176))+ (cEmissionScale.y * tmpvar_178))));
